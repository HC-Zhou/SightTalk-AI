...
async def test_context_async_prompt_injects_local_markdown_memory_without_query(tmp_path) -> None:
...
async def test_context_consolidates_short_term_memory_to_markdown_history(tmp_path) -> None:
    memory = LocalMarkdownMemory(tmp_path)
    context = AgentSessionContext(
        session_id="room-1",
        user_id="user_1",
    ...
    history = (
        tmp_path / "markdown_memory" / "sighttalk" / "user_1" / "HISTORY.md"
